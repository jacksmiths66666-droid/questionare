...
- [ ] **Step 2: Calculate Mahalanobis distance within comparable response blocks**

不把 68 国、126 题混在一起算。仅在同一国家、同一同质量表题块内计算，例如 `TRUST_SCI_*`、`SCIPOP_*`、`OUTSPOKEN_*`、`SDO_*`。缺失值以该国该题块均值临时填补用于计算；输出距离、所在题块和阈值触发原因。
...
- [ ] **Step 5: Set thresholds transparently**

不照抄旧代码的 `8.5`、`0.10`、`12`。先输出不同阈值下各国/各量表被标记比例，再结合 TISP 题块长度和样本分布确定本次阈值，并把选择理由写入 `tisp-screening-rules.md`。
